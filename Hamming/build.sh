...
echo " 1) Build encoder"
make -C ./tools/encoder/ 1>/dev/null
if [ $? -ne 0 ]; then
	echo "[-] Fail"
	exit
else
...
echo " 2) Prepare image (add description text)"
...
echo " 3) Encode image with Hamming-(15, 11)"
